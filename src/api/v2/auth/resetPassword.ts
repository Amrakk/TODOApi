...
export default async function resetPassword(req: Request, res: Response) {
    ...
    if (!email || !otp || !password) return res.status(400).json({ message: "Invalid credentials" });
    ...
    if (!isVerified) return res.status(403).json({ message: "Invalid OTP or email" });

    const salt = bcrypt.genSaltSync(10);
    const hash = bcrypt.hashSync(password, salt);

    const isUpdated = await database.updatePassword(email, hash);
    if (!isUpdated) return res.status(500).json({ message: "Error updating password" });
