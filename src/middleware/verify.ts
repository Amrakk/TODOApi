...
export default async function verify(req: Request, res: Response, next: NextFunction) {
    ...
    let id = "";
    const access_payload = valToken(access_token, process.env.ACCESS_SECRET_KEY as string);
    if (!access_payload) return res.status(401).json({ message: "Invalid token" });
    if (access_payload === "expired") {
        const ref_token = req.cookies.ref_token ?? null;
        if (!ref_token) return res.status(401).json({ message: "Invalid token" });

        const ref_payload = valToken(ref_token, process.env.REFRESH_SECRET_KEY as string);
        if (!ref_payload || ref_payload === "expired" || !(await cache.verifyRefToken(ref_payload.id, ref_token)))
